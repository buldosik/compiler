...
class Variable:
    def __init__(self, memory_offset, isInitialized=False, isIterator=False):
        self.memory_offset = memory_offset
        self.isInitialized = isInitialized
        self.isIterator = isIterator
...
class Procedure():
    ...
    def add_iterator(self, name):
        if name in self.symbols or name in self.links:
            return
        self.symbols.setdefault(name, Variable(self.last_indeks, True, True))
        self.last_indeks += 2
